...
var DEFAULT_HOSTNAME = '10.0.2.15';
...
/*
var do_respond_to_an_TESSEL_request = function(req,res){
  var img_data='';
  ...
  req.on('end',function(){
    console.log(new Buffer(img_data, 'binary'),'qweqwe');
  });
};
*/
    ...
    var responder = function (respbody, respheaders) {
      ...
      if (respheaders)
        for (var header_name of Object.keys(respheaders))
          res.setHeader(header_name, respheaders[header_name]);       //sets a single header value for implicit headers
      res.write(respbody);      //send a chunk of response body
      res.end();
    };
    console.log(req.connection.remoteAddress + ' ' + timestamp_str() +
                ' >>>> "' + pattern + '"');
    if (typeof reqhandlers[pattern] === 'function')
      reqhandlers[pattern](responder, reqbody, reqheaders, timestamp_str());
    else {
      res.statusCode = 400;
